...
def require_admin():
    if not session.get("admin"):
        ...
        raise HTTPException(response=redirect(url_for("admin_login")))
...
def admin_restaurants():
    ...
    return render_template("admin/restaurants.html", restaurants=restaurants, pending_users=pending_users)

@app.route("/admin/portal-user/<int:uid>/approve", methods=["POST"])
...
@app.route("/admin/portal-user/<int:uid>/reject", methods=["POST"])
...
def require_portal_login():
    if not session.get("portal_user_id"):
        from werkzeug.exceptions import HTTPException
        raise HTTPException(response=redirect(url_for("portal_login")))
...
@app.route("/portal/restaurant/edit", methods=["GET", "POST"])
def portal_restaurant_edit():
    require_portal_login()
    user = get_portal_user()
    restaurant = user.restaurant
    if not restaurant:
...
@app.route("/portal/tables", methods=["GET"])
def portal_tables():
    require_portal_login()
    user = get_portal_user()
    restaurant = user.restaurant
...
@app.route("/portal/table/add", methods=["POST"])
def portal_table_add():
    require_portal_login()
    user = get_portal_user()
    restaurant = user.restaurant
    t = Table(
    ...
    db.session.commit()
    flash("Table added!", "success")
    return redirect(url_for("portal_tables"))

@app.route("/portal/table/<int:tid>/edit", methods=["POST"])
def portal_table_edit(tid):
    require_portal_login()
    user = get_portal_user()
...
@app.route("/portal/table/<int:tid>/delete", methods=["POST"])
def portal_table_delete(tid):
    require_portal_login()
    user = get_portal_user()
    t = Table.query.get_or_404(tid)
    if t.restaurant_id != user.restaurant_id:
...
def portal_reservations():
    ...
    reservations = query.order_by(Reservation.reservation_date.desc(), Reservation.reservation_time).all()
    return render_template("portal/reservations.html",
                           restaurant=restaurant, reservations=reservations, as_money=as_money)

@app.route("/portal/reservation/<int:res_id>/status", methods=["POST"])
def portal_reservation_status(res_id):
    require_portal_login()
    user = get_portal_user()
    ...
    return redirect(url_for("portal_reservations"))

@app.route("/portal/no-show-stats")
def portal_no_show_stats():
    require_portal_login()
    user = get_portal_user()
    restaurant = user.restaurant
    if not restaurant:
